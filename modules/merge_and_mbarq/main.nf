...
process pre_mbarq_qc_process {
    ...
    tag { comparison_name }
    publishDir "${params.outdir}/pre_mbarq_qc", mode: 'copy'
    ...
    script:
    """
    # TODO: add pre-mbarq QC steps here

    {
      echo "### pre_mbarq_qc_process for ${comparison_name} ###"
      echo "merged_matrices_path=${merged_matrices_path}"
      echo "mbarq_meta_path=${mbarq_meta_path}"
    } > ${comparison_name}.pre_mbarq_qc.log.txt
    """
}
...
workflow merge_and_analyze {
  ...
  main:
    ...
    ordered_logs = mr.merge_log_ch
                    ...
                    .join(pre_mbarq_qc_log_ch)   // [id, merge_log, create_meta_log, pre_qc_log]
                    .join(filter_log_ch)         // [id, merge_log, create_meta_log, pre_qc_log, filter_log]
                    .join(mbarq_log_ch)          // [id, ..., mbarq_log]
                    .map { id, merge_log, meta_log, pre_qc_log, filter_log, mbarq_log ->
                        tuple(id, [merge_log, meta_log, pre_qc_log, filter_log, mbarq_log])
                    }
